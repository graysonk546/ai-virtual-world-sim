...
public final class Util {
    ...
    /**
     * Gernerate random integer in the interval [min, max).
     * @param max exclusive maximum boundary
     * @param min inclusive minimum boundart
     ...
     * @throws IllegalArgumentException if location is empty or is invalid
     */
    public static Item itemAtLocation(World world, Location location) throws IllegalArgumentException{
        Item result = null;
        for (Item item : world.getItems()){
            if (item.getLocation().equals(location)){
                result = item;
        ...
        if(result == null){
            throw new IllegalArgumentException("Location is empty or is invalid");
        }
        return result;
    }

    /**
     * copy of search surroundings in worldImpl
     */
    public static Set<Item> searchSurroundings(Location loc, int range, Iterable<Item> items) {
